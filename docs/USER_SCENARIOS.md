...
**Связанные сценарии:** `SUB-05`, `RCV-01`, `RCV-02`, `RCV-03`, `RCV-04`
...
**Связанные сценарии:** `DRF-01`, `DRF-02`, `DRF-03`, `DRF-04`, `SUB-01`, `SUB-03`, `SAF-04`
...
- **Распознаваем:** товар, количество, единица, поставщик, комментарий
- **Подтверждение:** Не нужно для понятной позиции: товар добавляется только в черновик. Неоднозначность требует уточнения.
- **Изменение состояния:** В черновик попадают только произнесённые товары и количества.
- **Восстановление:** Сохранить уже распознанные позиции и попросить повторить только непонятную часть.
...
**Результат:** Количество записывается в соответствующие колонки «Зал», «Бар» и «Кухня», а поставщик определяется по найденной строке актуального каталога. Строки без заказа не добавляются.
...
- **Распознаваем:** товар, количество, единица, поставщик, комментарий
- **Подтверждение:** Не нужно для понятной позиции: товар добавляется только в черновик. Неоднозначность требует уточнения.
- **Изменение состояния:** Количество записывается в соответствующие колонки «Зал», «Бар» и «Кухня», а поставщик определяется по найденной строке актуального каталога. Строки без заказа не добавляются.
- **Восстановление:** Сохранить уже распознанные позиции и попросить повторить только непонятную часть.
...
- **Распознаваем:** товар, количество, единица, поставщик, комментарий
- **Подтверждение:** Не нужно для понятной позиции: товар добавляется только в черновик. Неоднозначность требует уточнения.
- **Изменение состояния:** Позиции с количеством попадают в черновик; бот не придумывает отсутствующие значения.
- **Восстановление:** Сохранить уже распознанные позиции и попросить повторить только непонятную часть.
...
- **Распознаваем:** черновик, позиция, номер варианта, команда управления
- **Подтверждение:** Нужно перед очисткой непустого черновика или началом новой заявки вместо него.
- **Изменение состояния:** Новые позиции добавляются к текущей заявке.
- **Восстановление:** При отказе и непонятной команде оставить черновик без изменений.
...
**Ответ бота:** Сначала проверяет незавершённые позиции и показывает итоговую карточку. Большая заявка разбивается на страницы с кнопками «Назад» и «Далее».

**Результат:** До явного финального подтверждения запись в таблицу заказа не запускается; пользователь может просмотреть все страницы заявки.
...
- **Изменение состояния:** Черновик переходит к финальной проверке, но данные в Google Sheets ещё не меняются.
...
- **Доступ:** Пользователь подключён к заведению и видит только заявки своего заведения.
- **Распознаваем:** заявка, поставщик, сумма, статус, дата поставки
- **Подтверждение:** Нужно выбрать «Добавить товары» либо «Отправить как есть» для каждого предупреждения.
...
- **Восстановление:** Если запись точно не началась, можно повторить. При неизвестном результате бот не должен создавать возможный дубль.
...
- **Распознаваем:** заявка, поставщик, сумма, статус, дата поставки
- **Подтверждение:** Нужно перед записью заявки в таблицу и перед продолжением при предупреждении о минимальной сумме.
- **Изменение состояния:** Неоднозначный сетевой ответ не приводит к автоматическому дублю заказа поставщикам.
- **Восстановление:** До записи разрешить безопасный повтор; после неоднозначного результата не создавать возможный дубль.
...
_Версия каталога: 2.0. Категорий: 7._
