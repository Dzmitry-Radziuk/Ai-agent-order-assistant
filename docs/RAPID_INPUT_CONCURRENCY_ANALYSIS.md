...
| order history/status | Основной путь получает revision | `SubmissionService.send_status()` использует `submission_presenter` с raw `v2:order`, `v2:orderspage`, `v2:orderitems`; worker не инкрементирует revision | UNPROTECTED для stale status card; действие не является irreversible mutation |
