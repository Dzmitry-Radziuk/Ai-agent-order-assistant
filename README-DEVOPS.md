...
| `GOOGLE_ORDER_SUBMISSION_ENABLED` | Да | Нет | `false`: текущий production-процесс с ручной отправкой; `true`: будущая автоматическая отправка |
...
При `GOOGLE_ORDER_SUBMISSION_ENABLED=false` после кнопки «Отправить в таблицу заказа»
или равнозначной текстовой/голосовой команды бот записывает количества и комментарии
в лист `Заявка` конкретного заведения и вызывает настроенный Apps Script перерасчёта.
Затем заявка завершается локально без вызова центрального Apps Script отправки:
её вручную отправляет ответственный сотрудник. После ручной отправки лист `История`
становится источником реальных статусов. Экран «Мои заявки» фильтрует строки по
текущему заведению, показывает пять последних заявок и позволяет открыть нужную
кнопкой или голосовой командой. Более старые заявки доступны по страницам.

Значение `true` разрешено устанавливать только после отдельного согласования
автоматической отправки. Тогда после записи и перерасчёта бот дополнительно вызовет
центральный Apps Script.
...
При включённом Langfuse бот передаёт отдельные `generation`-наблюдения для
разбора текста и фото, сопоставления каталога, голосовой навигации и
транскрибации. В наблюдение входят модель, фактические токены и технические
...
| Модель | Match pattern | Input, USD/token | Output, USD/token |
|---|---|---:|---:|
| `gpt-4o-mini-transcribe` | `(?i)^(gpt-4o-mini-transcribe)$` | `0.00000125` | `0.000005` |
| `gpt-4o-transcribe` | `(?i)^(gpt-4o-transcribe)$` | `0.0000025` | `0.00001` |

Изменение Model Definitions влияет только на новые наблюдения. Проверка
...
7. Открыть черновик.
8. Подготовить заявку для ручной отправки.
9. Проверить запись, комментарий и перерасчёт в Google Sheets.
10. Открыть «Мои заявки», выбрать вторую заявку кнопкой и голосом.
...
- [ ] `GOOGLE_ORDER_SUBMISSION_ENABLED=false`, пока заявки отправляет ответственный сотрудник.
- [ ] Центральный Apps Script ботом не вызван.
- [ ] После ручной отправки лист `История` обновился.
- [ ] «Мои заявки» показывает только строки текущего заведения из листа `История`.
- [ ] Пять последних заявок, выбор кнопкой/голосом и переход между страницами работают.
