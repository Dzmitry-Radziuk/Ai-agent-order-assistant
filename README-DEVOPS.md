...
| `GOOGLE_ORDER_SUBMISSION_ENABLED` | Да | Нет | `false`: записать и пересчитать таблицу заведения без центральной отправки; production — `true` |
...
поставщики и центральная таблица её не получают. Для ручной проверки экран статусов
может прочитать последнюю заявку из готового листа `История`. При значении `true`
после тех же записи и перерасчёта выполняется центральная отправка, а тестовый
fallback статусов отключается автоматически.
