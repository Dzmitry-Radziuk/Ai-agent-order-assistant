...
При `GOOGLE_ORDER_SUBMISSION_ENABLED=false` бот ничего не записывает при финальном
подтверждении. Для ручной локальной проверки экран статусов может прочитать последнюю
заявку из готового листа `История`, только если в сессии ещё нет собственных номеров.
При значении `true` этот тестовый fallback отключён автоматически.
