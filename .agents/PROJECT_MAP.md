...
| `integrations/openai_client.py`, `integrations/openai_*_prompt.py` | Транскрипция, разбор текста и фото, сопоставление ИИ, prompt-контракты и наблюдаемость |
...
| Распознавание голоса и фото | `input/media_recognition.py`, `input/voice_policy.py`, `input/photo_ingestion.py`, `input/photo_observation_reconciliation.py` | Медиа, прогресс, сверка наблюдений и политика без владения бизнес-правилами |
...
| Текстовые команды | `parsing/commands/` | Нормализация, навигация, выбор подразделения, товары, комментарии и порядок маршрутизации |
...
| Надёжная координация | `services/orchestrator.py`, `services/orchestrator_analytics.py` | Захват, аренда, контрольные точки, композиция среды выполнения и аналитические события |
| Машина состояний | `services/engine.py`, `services/engine_quantity_flow.py`, `services/engine_submission_preparation.py` | Авторитетный порядок переходов, поток количества, подготовка снимка заявки и переходный мост |
| Отправка и статусы | `services/submission.py`, `services/submission_*.py` | Координация заявки и отдельные протоколы доступа, статусов, product-add, уведомлений, пересчёта и каталоговых контрольных точек |
...
Крупные координаторы разделяются только по доказанной границе ответственности:
structured vision-наблюдения вынесены из OpenAI-транспорта, prompt-контракты
сохранены побайтно в тематических модулях, а submission-протоколы доступа,
статусов, product-add, перерасчёта и уведомления о завершении имеют собственные
сервисы-адаптеры. Аналитика, поток количества и подготовка заявки также вынесены
в тематические сервисы. `orchestrator.py`, `engine.py` и оставшаяся часть
`submission.py` сохраняют только доказанные границы координации контрольных точек,
переходов и внешних эффектов.
