...
flowchart LR
    ...
    ORCH --> INPUT[Text / Voice / Photo parsing]
...
| Модуль | Роль |
|---|---|
| `services/orchestrator.py` | Сквозной pipeline: claim update, регистрация и доступ, блокировка чата, загрузка состояния, распознавание входа, каталог, engine, сохранение, задачи и доставка ответа |
...
| `services/parser.py` | Быстрые и детерминированные команды, количества и intent до или после AI-разбора |
...
4. Voice сначала транскрибируется; photo распознаётся по контракту таблицы; text разбирается напрямую.
5. Детерминированные правила защищают навигационные и отрицательные фразы от превращения в товары.
...
5. Количества и комментарии записываются в лист заказа.
