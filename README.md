...
10. После кнопки «Отправить в таблицу заказа» или равнозначной текстовой/голосовой команды worker обновляет количество и комментарии в листе `Заявка`, затем вызывает Apps Script перерасчёта.
11. Ответственный сотрудник вручную отправляет подготовленную заявку поставщикам. Центральный Apps Script бот не вызывает, пока `GOOGLE_ORDER_SUBMISSION_ENABLED=false`.
12. После ручной отправки лист `История` обновляется, а бот читает оттуда реальные статусы заявки.
