...
- распознаёт списки товаров на фотографиях;
- понимает разговорные голосовые команды на разных этапах диалога;
...
- сохраняет комментарий к товару и общий комментарий к заявке;
...
10. После подтверждения worker записывает заявку в Google Sheets, обновляет каталог и вызывает Apps Script перерасчёта.
11. Пользователь получает результат отправки и кнопку просмотра своих заявок.
...
├── Dockerfile               # production-образ Python
...
1. запись строк в историю;
2. обновление количества и комментариев в каталоге;
3. вызов Apps Script;
4. фиксация финального состояния;
