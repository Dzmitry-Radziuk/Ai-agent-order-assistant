...
- в исходной таблице заказа берёт количество только из колонок «Зал», «Бар» и «Кухня», пропускает строки без заказа, а поставщика определяет по актуальному каталогу, не по фотографии;
...
- после записи в таблицу позволяет начать пустую новую заявку, а при активном черновике сначала просит подтверждение;
- не подставляет конкретный товар по слишком общему запросу вроде «говядина»;
- показывает похожие варианты и позволяет выбрать номером или названием, но не добавляет слабое совпадение автоматически;
...
- надёжно записывает заявку в таблицу и завершает черновик только после успешного перерасчёта;
...
1. Telegram отправляет update на HTTPS webhook в `api`.
2. API проверяет секретный заголовок, идемпотентно сохраняет update в PostgreSQL и
   передаёт его Celery worker через Redis.
3. Durable-координатор в `services/orchestrator.py` получает lease чата, загружает
   привязку заведения и состояние, а затем ведёт checkpoints результата, ответа и
   фоновых задач.
4. `input` переводит Telegram update в нейтральный вход. Текст проходит общий
   semantic pipeline; голос сначала транскрибируется, фотография распознаётся
   vision-моделью.
5. `parsing` объединяет deterministic extraction и структурированный ответ OpenAI,
   причём исходный текст остаётся доказательством количества, товара и комментария.
6. `application` вызывает общий conversation use case. `conversation`, `catalog` и
   `orders` принимают решения по state machine, товару, количеству и правилам заказа.
7. Состояние и результат сохраняются до внешней доставки. `presentation` строит
   Telegram-карточку, а интеграция отправляет или редактирует сообщение.
8. После явного подтверждения `services/submission.py` повторно читает актуальные
   данные, проверяет доступ, записывает лист `Заявка`, сверяет результат и запускает
   Apps Script перерасчёта.
9. Ответственный сотрудник вручную отправляет подготовленную заявку поставщикам.
   Центральный Apps Script не вызывается, пока
   `GOOGLE_ORDER_SUBMISSION_ENABLED=false`.
10. Лист `История` остаётся источником реальных статусов отправленных заявок.
...
### Статус многоканальности

Контракт входа, семантические действия, parsing, state policy, conversation use case,
catalog и order rules уже не зависят от Telegram. Реализован и протестирован только
Telegram-adapter. `ConversationEngine` пока возвращает совместимый
`EngineResult`/`BotReply`, а application use case преобразует его строки в нейтральное
представление через внедрённый mapper. Это защищённый legacy bridge, а не доказанная
поддержка MAX, Web или REST.

Для нового канала нужны собственные входной adapter, преобразователь действий,
renderer ответа и delivery/effect adapter. Удалять bridge безопасно после появления
одного подтверждённого вертикального сценария второго канала, а не ради чистоты
структуры Telegram-приложения.

### Статус каталога и масштаба

Core использует порт `CatalogSearch`; `ListCatalogSearch` сохраняет текущую работу с
полным каталогом из Google Sheets/Redis и подтверждает возможность bounded provider.
Однако production runtime по-прежнему загружает и копирует список, а ranking выполняет
линейный проход. Тест на 100 000 позиций доказывает границу интерфейса, но не является
нагрузочным тестом production.

Перед реальным каталогом такого размера нужен индексированный, ограниченный по
заведению и поставщику backend (например PostgreSQL или API), его подключение в runtime
composition и benchmark времени, памяти и качества выдачи. Conversation core и safety
gates для этого переписывать не требуется.
...
├── src/restaurant_bot/
│   ├── api/                 # FastAPI, webhook и health endpoints
│   ├── application/         # нейтральные use cases, порты и контракты каналов
│   ├── catalog/             # evidence, retrieval, scoring и safety сопоставления
│   ├── conversation/        # state policy, переходы и чистые решения диалога
│   ├── domain/              # модели команд, состояния, товаров и ответов
│   ├── input/               # преобразование Telegram update в нейтральный вход
│   ├── integrations/        # клиенты Telegram, OpenAI, Google Sheets, Redis и Apps Script
│   ├── observability/       # структурированные события и безопасные метаданные
│   ├── orders/              # правила количества, поставщика и разрешения позиции
│   ├── parsing/             # deterministic parsing и AI reconciliation
│   ├── persistence/         # общие транзакционные и lifecycle-контракты
│   ├── presentation/        # Telegram-тексты, кнопки и отображение нейтрального результата
│   ├── repositories/        # транзакционный доступ к PostgreSQL
│   ├── services/            # защищённые runtime/effect coordinators, а не общий core
│   ├── venues/              # коды, контракты и правила доступа заведений
│   └── workers/             # Celery entrypoints и фоновые задачи
...
В текущем ручном процессе центральная отправка выключена:
`GOOGLE_ORDER_SUBMISSION_ENABLED=false`. Финальное подтверждение записывает
количества и комментарии в лист `Заявка`, запускает перерасчёт и завершает
черновик. Затем ответственный сотрудник вручную отправляет подготовленную заявку.
Центральный Apps Script бот не вызывает.

Экран статусов работает только на чтение. Бот фильтрует лист `История` по текущему
заведению и показывает пять последних реально отправленных заявок. Пользователь
выбирает заявку кнопкой или голосовой фразой — например, «Покажи вторую». Внутри
показываются поставщики, товары и статусы. Более старые заявки доступны постранично.

Значение `GOOGLE_ORDER_SUBMISSION_ENABLED=true` зарезервировано для будущего
подключения автоматической отправки через центральный Apps Script.
...
Сам push подтверждает только обновление ветки. Успешное развёртывание подтверждают
завершившийся `deploy-dev` и health-check приложения.
