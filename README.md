...
оставляет черновик без изменений и не запускает запись в таблицы. Реальную отправку
нужно включать явно и только в разрешённом окружении.
...
явной пометкой «Тестовые данные». Это позволяет проверять чтение статусов без записи
в Google Sheets. После переключения `GOOGLE_ORDER_SUBMISSION_ENABLED=true` тестовый
