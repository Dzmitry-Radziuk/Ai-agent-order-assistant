...
def _collapse_source_reference_variants(
    ...
    if len(deterministic) == 1:
        ...
        if len(source_keys) != 1 or not normalized_source:
            return items
        # When the deterministic parser cannot split a packaging-heavy voice
        # line, a long shared prefix is the only safe source-occurrence anchor.
        # Short/common prefixes remain untouched and are handled by clarification.
