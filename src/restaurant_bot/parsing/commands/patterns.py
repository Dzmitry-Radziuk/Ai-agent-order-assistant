...
_NATURAL_COMMANDS: list[tuple[Intent, re.Pattern[str]]] = [
    ...
    (
        Intent.CLEAR_CART,
        re.compile(
            r"^(?:сброс|(?:сбрось|сбросить|брось|бросить|очисти|очистить|обнули|обнулить|вычисти|вычистить)"
    ...
    (
        ...
        re.compile(
            ...
            r"(?:(?:давай|давайте|пожалуйста|готово|всё|все|ладно|хорошо)\s+)*"
            r"(?:добавь|добавить|добавим|положи|положить|перенеси|перенести)"
            r"(?: (?:товар|товары|позицию|позиции|заявку|заказ))?"
            r"(?: в (?:корзину|заявку))?\s+и\s+"
            r"(?:проверь|проверить|посмотри|посмотреть|покажи|показать)"
            r"(?: (?:итог|итоги|заявку|заказ|перед отправкой))?)$",
            re.I,
        ),
    ),
    (
