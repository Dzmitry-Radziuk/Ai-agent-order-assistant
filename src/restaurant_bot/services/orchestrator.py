...
from restaurant_bot.services.orchestrator_analytics import OrchestratorAnalyticsService
...
class UpdateOrchestrator:
    ...
    def _scenario_for_intent(intent: Intent) -> str:
        """Передаёт определение сценария сервису аналитики."""
        return OrchestratorAnalyticsService._scenario_for_intent(intent)
    ...
    def _effective_analytics_command(
        ...
        """Передаёт восстановление контекстного количества сервису аналитики."""
        return OrchestratorAnalyticsService._effective_analytics_command(
            event,
            command,
            state,
    ...
    def _request_analytics(
        ...
        *,
        ...
        """Передаёт формирование итога запроса сервису аналитики."""
        return OrchestratorAnalyticsService(self).request_analytics(
            event,
            command,
            state,
            previous_stage=previous_stage,
    ...
    def _analytics_request_text(event: TelegramEvent, command: ParsedCommand) -> str:
        """Передаёт выбор диагностического текста сервису аналитики."""
        return OrchestratorAnalyticsService._analytics_request_text(event, command)
    ...
    def _record_request_outcome(
        ...
        *,
        ...
        """Передаёт запись итога запроса сервису аналитики."""
        OrchestratorAnalyticsService(self).record_request_outcome(
            trace,
            log,
            analytics,
            chat_id=chat_id,
