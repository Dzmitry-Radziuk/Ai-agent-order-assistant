"""Содержит политики разрешения товарных и item-resolution modal-контекстов."""
...
class ItemResolutionPolicyMixin:
    """Группирует политики разрешения товарных modal-состояний."""
    ...
    def _evaluate_manual_details(
        ...
        if command.intent is Intent.UNKNOWN:
            return CompatibilityDecision(CompatibilityAction.AMBIGUOUS)
        if command.intent in self._INTERRUPT_INTENTS:  # type: ignore[attr-defined]
            return CompatibilityDecision(CompatibilityAction.INTERRUPT)
        return CompatibilityDecision(CompatibilityAction.INTERRUPT)
    ...
    def _evaluate_product_add_details(
        ...
        if command.intent is Intent.UNKNOWN:
            if self._has_product_add_description(command):
                return CompatibilityDecision(CompatibilityAction.CONTINUE)
            return CompatibilityDecision(CompatibilityAction.AMBIGUOUS)
        if command.intent in self._INTERRUPT_INTENTS or command.intent in {  # type: ignore[attr-defined]
            Intent.ADD_MORE,
