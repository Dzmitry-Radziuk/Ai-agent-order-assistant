"""Содержит политики подтверждения заявки и order/review modal-контекстов."""
...
class OrderFlowPolicyMixin:
    """Группирует политики подтверждения заявки и review."""
    ...
    def _evaluate_new_order_confirmation(
        ...
        if command.intent is Intent.ADD_ITEMS and self._has_product_items(command):  # type: ignore[attr-defined]
        ...
        if command.intent is Intent.CONFIRM and not self._has_product_items(command):  # type: ignore[attr-defined]
            return CompatibilityDecision(CompatibilityAction.CONTINUE, mode="yes")
        if command.dialogue_response is DialogueResponse.AFFIRM and not self._has_product_items(  # type: ignore[attr-defined]
            command
        ):
            return CompatibilityDecision(CompatibilityAction.CONTINUE, mode="yes")
        ...
        if command.intent in {
            ...
            return CompatibilityDecision(CompatibilityAction.INTERRUPT, mode="independent")
        if (
            command.dialogue_response is DialogueResponse.UNCERTAIN
            or command.intent is Intent.UNKNOWN
        ):
            return CompatibilityDecision(CompatibilityAction.AMBIGUOUS)
        return CompatibilityDecision(CompatibilityAction.INTERRUPT, mode="independent")
    ...
    def _evaluate_submission_failed(
        ...
        if (
            ...
            or command.dialogue_response is DialogueResponse.UNCERTAIN
        ):
    ...
    def _evaluate_add_more_confirm(
        ...
        if self.context_for(state) is not CompatibilityContext.ADD_MORE_CONFIRM:  # type: ignore[attr-defined]
            return CompatibilityDecision(CompatibilityAction.NOT_APPLICABLE)
    ...
    def _evaluate_submit_confirm(
        ...
        if not self._can_use_submit_confirm_context(state):
            return CompatibilityDecision(CompatibilityAction.NOT_APPLICABLE)
        if command.intent is Intent.ADD_ITEMS:
            if self._has_concrete_new_items(command):  # type: ignore[attr-defined]
                return CompatibilityDecision(CompatibilityAction.INTERRUPT)
            return CompatibilityDecision(CompatibilityAction.AMBIGUOUS)
