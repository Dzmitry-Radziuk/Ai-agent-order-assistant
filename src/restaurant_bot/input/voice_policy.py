...
def voice_transcription_prompt(state: ConversationState) -> str:
    ...
    if current and current.status == ItemStatus.AMBIGUOUS:
        ...
        return (
            "Русская речь. Выбор одного варианта товара: "
            f"{names}. Можно назвать номер («первый», «вариант два») "
            "или полное/частичное название. Верни только произнесённый текст."
        )
    if current and state.stage == SessionStage.AWAIT_UNIT_QUANTITY and current.catalog_unit:
        ...
        return (
            f"Русская речь. Количество товара «{product_name}» должно быть в {expected_unit}. "
            "Сохрани число и полную единицу; не путай «грамм» и «килограмм». "
            "Ничего не заменяй и не придумывай. Верни только произнесённый текст."
        )
    if current and current.status == ItemStatus.MISSING_QTY and current.catalog_unit:
        expected_unit = normalize_unit(current.catalog_unit)
        product_name = current.catalog_name or current.source_query
        return (
            f"Русская речь. Количество товара «{product_name}» в {expected_unit}: "
            "сохрани только число («пять», «5») и единицу («пять штук», «5 кг»). "
            "Короткий ответ — это количество, а не «не добавлять». "
            "Не заменяй и не придумывай текст. Верни только произнесённый текст."
        )
    ...
    return (
        "Русская речь сотрудника кафе. Верни только дословный русский текст; "
        "ничего не заменяй и не придумывай. Сохрани «не» и «нет»: они меняют действие. "
        "Команды могут быть «добавить товары», «покажи черновик», «отправить заявку», "
        "«очистить черновик», «не добавлять», «не отправлять», «пропускаем», "
        "«первый/второй вариант». Команда не является названием товара."
        f"{screen_hint}"
    )
