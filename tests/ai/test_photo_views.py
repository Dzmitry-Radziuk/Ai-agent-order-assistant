...
def test_wide_table_prepares_original_product_and_order_views(tmp_path: Path) -> None:
    """Готовит три view для широкой плотной таблицы."""
    photo = tmp_path / "wide.png"
    _save_image(photo, (1600, 900))

    preparation = prepare_photo_views(photo, "image/png")

    assert [view.name for view in preparation.views] == ["original", "product", "order"]
    ...
    assert preparation.product_view_size == (2080, 1800)
    assert preparation.order_view_size == (2016, 1800)
    assert preparation.views[1].mime_type == "image/png"
    assert all(view.width > 0 and view.height > 0 for view in preparation.views)


def test_table_views_preserve_crop_aspect_ratio_and_overlap(tmp_path: Path) -> None:
    """Сохраняет пропорции увеличенных crops и перекрытие product/order областей."""
    photo = tmp_path / "wide.png"
    _save_image(photo, (1600, 900))

    preparation = prepare_photo_views(photo, "image/png")
    product = preparation.views[1]
    order = preparation.views[2]

    assert product.width / product.height == pytest.approx((1600 * 0.65) / 900)
    assert order.width / order.height == pytest.approx((1600 * 0.63) / 900)
    assert product.width + order.width > 1600 * preparation.upscale_factor
