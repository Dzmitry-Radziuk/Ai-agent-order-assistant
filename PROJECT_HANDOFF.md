...
## Текущая задача — изменение количества у похожих позиций

По журналам voice-update `247309456` и `247309457` транскрипция и
детерминированный parser корректно выделяли команду изменения количества:
цель `горчицы дижонской`, новое количество `10 шт`. В черновике одновременно
находились `Горчица дижонская` и `Горчица дижонская большое зерно`.

Первой неверной границей был `conversation.selection.find_cart_item`: прежняя
оценка учитывала только совпавшие слова, поэтому обе позиции получали одинаковый
балл и безопасный resolver возвращал `None`. Полное словесное название с учётом
падежных окончаний теперь имеет приоритет над более длинной позицией с
дополнительными отличительными признаками. Действительно общий запрос вроде
`горчицы` остаётся неоднозначным и не меняет ни один товар.

Добавлены положительная регрессия для пары похожих названий и отрицательная
регрессия для общего названия. Focused-набор: `115 passed`. Полный pytest:
`1728 passed`. Mypy проверил `167` исходных файлов; Ruff check и format проверили
`295` файлов. Проверка `41` пользовательского сценария, `38` Markdown-файлов,
compileall и `git diff --check` проходят. Контейнеры пересобраны из текущего
рабочего дерева: migrate завершился с кодом `0`, API и worker имеют статус
`healthy`, PostgreSQL и Redis — `healthy`, beat запущен.

## Предыдущая задача — достоверный прогресс фото и граница товарных позиций

По журналам photo-update `247309221` и `247309223` vision вернул пять
предварительных фрагментов, после чего catalog resolution обоснованно объединил
два соседних фрагмента одной строки и сформировал четыре правильные позиции
черновика. Промежуточная карточка при этом показывала
`Найдено позиций: 5`, потому что `InputRecognitionService` считал сырой
`ParsedCommand.items` до каталогового восстановления. Карточка теперь сообщает
только о завершении распознавания и начале сверки с каталогом; достоверный состав
и количество товаров показывает итоговый черновик.

Отдельно по text-update `247309417`, `247309420` и `247309424` подтверждён
обратный дефект: AI parsing и normalization правильно выделяли три полных
каталоговых названия, но `merge_catalog_qualified_items` склеивал первые два из-за
общего слабого fuzzy-кандидата. Восстановление фрагментов теперь запрещено, если
каждый соседний запрос уже полностью подтверждает отдельную каталоговую
идентичность. Полезное объединение действительно разорванного названия, включая
голосовой хвост `Relish`, сохранено.

Добавлена регрессия для списка `Хрен столовый 5кг`, `Горчица Домашняя…` и
`Свинина Окорок Пармский с/к`; обновлён контракт промежуточной photo-карточки.
Focused-набор: `36 passed`. Полный pytest: `1726 passed`. Mypy и Ruff для
изменённых файлов проходят. Полные mypy, Ruff check/format, проверка сценариев,
Markdown-ссылок, compileall и `git diff --check` проходят. Контейнеры пересобраны
из текущего рабочего дерева: migrate завершился с кодом `0`, API и worker имеют
статус `healthy`, PostgreSQL и Redis — `healthy`, beat запущен.

## Предыдущая задача — фото таблицы без номеров строк

Ручные повторы со скриншотами `1130–1137 × 595–606` показали, что номера строк
Google Sheets нельзя считать обязательной частью входа. Пользователь может
прислать обрезанный фрагмент без левой нумерации, без заголовков или таблицу
другого формата. Vision-модель при этом могла сама поставить
`sheet_row_numbers_visible=true` и придумать номера. Прежний контракт делал их
обязательными, запускал второй полный vision-проход и после сохранявшегося
конфликта отклонял корректно прочитанные товарные строки.

Теперь единственным обязательным геометрическим доказательством является одна
визуальная строка: товар, количество и комментарий должны быть прочитаны из неё.
Номер строки листа остаётся необязательной подсказкой и не получает авторитет
только из AI payload. Неподтверждённые номера не переносятся в `ExtractedItem`,
не участвуют в catalog resolution и не вызывают повторное распознавание.
Повторный vision-проход сохраняется только для реальной неопределённости области
заказа: заполненная ячейка не связана с товаром или модель явно сообщает о
непрочитанной потенциальной строке.

Photo prompt отдельно описывает фрагменты без заголовков и номеров строк:
такие таблицы разбираются по горизонтальной связи ячеек, без выдумывания
подразделения. Удалена лишняя загрузка каталога на этапе media recognition;
сопоставление с каталогом остаётся в существующем владельце catalog resolution.
Добавлены регрессии для обрезанной таблицы, ложного заявления AI о видимых
номерах и конфликта неподтверждённого номера с названием товара. Focused-набор
photo/input прошёл, полный pytest: `1725 passed`; mypy и Ruff проходят.
Контейнеры пересобраны из текущего кода: migrate завершился с кодом `0`, API
и worker имеют статус `healthy`, PostgreSQL и Redis — `healthy`, beat запущен.

## Предыдущая задача — проверка связи строки фото с товаром каталога

По журналу update `247309184` было подтверждено смещение строк при
распознавании уменьшенного скриншота Google Sheets. Catalog resolution теперь
не подменяет прочитанное название товаром из несовместимой строки. Последующее
ручное тестирование уточнило контракт: сама нумерация листа необязательна, поэтому
актуальное правило описано в текущем разделе выше.

## Предыдущая задача — расчёт кратности заказа

Колонка каталога «Минимальная Кратность в заказе» трактуется как шаг допустимого
количества: при значении `3` разрешены `3`, `6`, `9` и следующие кратные значения.
Рекомендация рассчитывается для количества текущей заявки и показывает полное
допустимое количество, которое пользователь выбирает в черновике. Значение,
уже находящееся в строке Google Sheets, не вычитается из рекомендации: для
`1 шт` при кратности `3` бот предлагает `3 шт`, а для `4 шт` — `6 шт`.

Первой неверной границей был
`conversation.quantity_resolution.suggested_quantity_for_multiple`: прежняя
формула складывала количество заявки со значением из таблицы, округляла сумму и
возвращала разницу. Поэтому при `existing_quantity=1`, количестве заявки `1` и
кратности `3` пользователь видел рекомендацию `2 шт`. Владелец правила теперь
округляет вверх только количество текущей заявки. Запись подтверждённой заявки
в Google Sheets по-прежнему выполняется существующим инкрементным механизмом;
его контракт в этой задаче не менялся.

Сообщение финальной проверки объясняет правило без слова «партия»: «Этот товар
можно заказать по 3 шт: 3 шт, 6 шт, 9 шт и так далее». Добавлены регрессии для
`1 → 3`, `4 → 6`, точного значения `6` и независимости рекомендации от текущей
ячейки таблицы. Focused-набор прошёл: `74 passed`. Полный pytest прошёл:
`1723 tests collected`, ошибок нет. Mypy, Ruff check/format, проверка сценариев,
Markdown-ссылок, compileall и `git diff --check` также прошли. Контейнеры после
этого изменения ещё не пересобирались; ручная Telegram-проверка остаётся нужна.

## Предыдущая задача — semantic help routing
...
## COMMENT-SCOPE-COMMAND-WRAPPER-2026-08-21

В голосовой и текстовой команде «Добавь общий комментарий привезти завтра к
восьми вечера» служебная оболочка попадала в комментарий позиции. Первая
неверная граница была в `strip_explicit_comment_scope_prefix`: функция удаляла
только слова области, но не глагол действия и фразу «общий комментарий».

Каноническая очистка области комментария теперь также удаляет командные формы
«добавь», «внеси», «запиши», «укажи», «помести» и «положи» перед словом
«комментарий». Этот же владелец используется при AI-reconciliation, поэтому
текстовый и голосовой маршруты сходятся, а в заявку сохраняется только
пожелание пользователя. Добавлена регрессия на обоих каналах.

Полный pytest, mypy, Ruff check, Markdown links, compileall и `git diff --check`
проходят. Контейнеры и внешние сервисы в этом изменении не запускались.

## QUANTITY-CATALOG-PROVISIONAL-2026-08-21

Для строки вроде «Утка Жир Кусок 10 кг, желательно на завтра» ИИ правильно
возвращал количество, но `restore_explicit_order_terms` удалял его до выбора
кандидата каталога: единственный числовой фрагмент ошибочно считался измерением.
Это была первая неверная граница обработки.

Теперь одно совпадающее количество сохраняется как предварительное значение до
каталожной проверки. Единый владелец `reconcile_order_quantity_evidence` затем
отбрасывает число, если оно подтверждено только названием каталога (например,
«Тесто слоёное дрожжевое 10 кг»), и сохраняет его, если в исходной фразе есть
отдельное количество заказа (например, «Утка Жир Кусок 10 кг, желательно на
завтра»). Правило не зависит от конкретного товара или языка его названия.

Добавлены регрессии для восстановления количества, text/voice-пути и фасовки
теста в каталоге. Полный pytest, mypy, Ruff check/format, Markdown links,
compileall и `git diff --check` проходят. Контейнеры и внешние сервисы в этом
изменении не запускались.
...
## PHOTO-COMMENT-CORRECTION-2026-08-21

Исправлена граница комментария для уже найденной позиции: точная фраза названия
товара сохраняется как цель, а морфологические фрагменты снова разбираются по
существующему алгоритму и не попадают в комментарий. Добавлена регрессия для
«Хрен столовый» и сохранены проверки текстового и голосового маршрутов.

Для фото таблиц с неопределённой заполненной ячейкой добавлен один ограниченный
повторный vision-запрос с оригиналом и увеличенным видом. Повтор принимается только
при полном результате; если неопределённость сохраняется, действует прежний
безопасный отказ без добавления неподтверждённых товаров. Проверен сценарий фото
таблицы с двумя видимыми вариантами представления.
...
## PHOTO-ROW-IDENTITY-CORRECTION-2026-08-21

По журналу update `247309048` подтверждена первая неверная граница: vision-модель
вернула количество из одной строки вместе с названием соседнего товара. При этом все
`sheet_row_number` были пустыми, а результат ошибочно считался полным, поэтому повторное
распознавание не запускалось. Каталожный поиск и запись в черновик уже получили
перепутанную позицию и не являлись источником ошибки.

Дополнительное сравнение update `247309061` и `247309062` показало вторую проблему.
Сжатый скриншот `1280x522` проходил два полных vision-запроса и завершался безопасным
отказом через `143725` мс, потому что модель должна была вернуть номера всех видимых,
включая пустые, строк. Более крупный вариант за один проход правильно связал строки `8`
и `12`, но всё равно занял `73536` мс. Это подтвердило, что каталог и последующая
маршрутизация не были источником ошибки: лишнее время и отказ возникали на границе
подготовки изображения и контракта vision-ответа.

Для скриншотов Google Sheets видимая нумерация строк остаётся проверяемой координатой
товара, количества и комментария, но теперь обязательна только для строк с фактическим
или возможным заказом. Пустые строки не сериализуются, поэтому допустимы возрастающие
номера с пропусками, например `8` и `12`; дубли и обратный порядок по-прежнему запрещены.
Повторный проход выполняется только при реальной неопределённости строк заказа и для
таблицы получает один подготовленный вид без дублирования полного оригинала.

Распознавание больше не зависит от жёсткой минимальной ширины скриншота. Табличная
структура определяется по относительной геометрии и цветам, масштаб выбирается по
реальному размеру кадра, а небольшие обычные фотографии списков также адаптивно
увеличиваются. В vision-запросе возвращаются только заполненные строки заказа, включён
минимальный режим рассуждений и уменьшен объём ответа; полная область заказа при этом
по-прежнему просматривается, а количество принимается только из той же визуальной строки.
Фрагменты без заголовков «Зал», «Бар» и «Кухня» сохраняют безопасный маршрут без
выдумывания подразделения.

При первом запуске обновлённого runtime update `247309073` и `247309075` выявили
несовместимость параметра краткости с OpenAI Responses API: SDK отправлял верхнеуровневый
`verbosity`, который API отвергал кодом `400`. Параметр перенесён в поддерживаемый контракт
`text.verbosity`; после исправления полный набор тестов повторно прошёл.

Добавлены регрессии для размеров `971x477` и `480x236`, адаптивного увеличения обычного
списка, разреженных строк `8` и `12`, компактного vision-запроса и прежних проверок
безопасного отказа. Полный pytest прошёл: `1720 passed`; mypy, Ruff check/format,
проверка сценариев, Markdown-ссылок, compileall и `git diff --check` также проходят.
Контейнеры повторно пересобраны после исправления SDK-контракта: API и worker имеют
статус `healthy`, PostgreSQL и Redis — `healthy`, beat запущен, migrate завершился с
кодом `0`. После запуска update `247309076` успешно прошёл один vision-запрос за `6505`
мс и весь пользовательский маршрут за `10535` мс без ошибки API. Это подтверждает
работоспособность нового быстрого запроса на обычном фото; ручной повтор исходного
небольшого скриншота таблицы остаётся обязательной приёмочной проверкой его геометрии.

Ручной повтор update `247309184` выявил, что одних структурных признаков номера
строки недостаточно: модель могла вернуть уникальные возрастающие номера, но
связать их с названиями из соседних строк. Контракт усилен семантической сверкой
пары «номер строки + название из фото» с текущим каталогом заведения. При
противоречии выполняется один повтор чтения; повторный конфликт завершает
обработку без добавления товаров. Даже если противоречивое наблюдение дойдёт до
catalog resolution, номер строки больше не может подменить несовместимое
название товара. После исправления focused-набор прошёл (`145 passed`), полный
pytest прошёл (`1725 passed`), статические и документационные проверки зелёные.
После исправления контейнеры пересобраны; API и worker имеют статус `healthy`,
PostgreSQL и Redis — `healthy`, beat запущен, migrate завершился с кодом `0`.
